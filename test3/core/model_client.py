...
Bạn là trợ lý AI chăm sóc khách hàng bảo hiểm. 
...
class QwenOmniClient:
    ...
    def __init__(self, model_name="thaomike/qwen-finetune-research", device="auto"):
    ...
    def _get_input_device(self):
        ...
        if hasattr(self.model, "thinker") and hasattr(self.model.thinker, "device"):
            return self.model.thinker.device
    ...
    def _decode_generated_text(self, generated_ids, prompt_length: int) -> str:
    ...
    def _text_content(self, text: str) -> list[dict]:
        return [{"type": "text", "text": text}]

    def _history_to_conversation(self, messages: list, audio_path: str | None = None) -> list[dict]:
        conversation = [{"role": "system", "content": self._text_content(SYSTEM_PROMPT)}]
        ...
        for msg in messages:
            ...
            if isinstance(content, list):
                ...
                if role == "tool":
                    ...
                    role = "user"
                    text = f"Kết quả từ tool {tool_name}: {text}"
                normalized_content = self._text_content(text)
        ...
        if audio_path and os.path.exists(audio_path):
            for entry in reversed(conversation):
                if entry["role"] == "user":
                    text_blocks = [block for block in entry["content"] if block.get("type") == "text"]
                    user_text = text_blocks[-1]["text"] if text_blocks else ""
                    entry["content"] = [
    ...
    def transcribe_audio(self, audio_path: str) -> tuple[str, float]:
        ...
        conversation = [
            {
                "role": "system",
                "content": self._text_content("You are an accurate Vietnamese speech transcription assistant."),
            },
            {
                "role": "user",
                "content": [
                    {"type": "audio", "path": audio_path},
                    {"type": "text", "text": "Hãy chép lại chính xác nội dung audio này bằng tiếng Việt."},
                ],
            },
        ]
        ...
        transcript = self._decode_generated_text(generated_ids, inputs["input_ids"].shape[1])
        latency = time.time() - t0
    ...
    def generate_response(self, messages: list, audio_path: str = None) -> tuple[str, float]:
        ...
        import torch
        conversation = self._history_to_conversation(messages, audio_path=audio_path)
        inputs = self._build_inputs(conversation)
        with torch.no_grad():
            generated_ids = self.model.generate(
                **inputs,
        ...
        raw = self._decode_generated_text(generated_ids, inputs["input_ids"].shape[1])
        latency = time.time() - t0
    ...
    def text_to_speech(self, text: str, output_path: str) -> tuple[str, float]:
        ...
        try:
            conversation = [
                {
                    "role": "system",
                    "content": self._text_content(self.AUDIO_SYSTEM_PROMPT),
                },
                {
                    "role": "user",
                    "content": self._text_content(f"Đọc to câu sau bằng tiếng Việt: {text}"),
                }
            ]
            import torch
            inputs = self._build_inputs(conversation)
            ...
            if audio is not None:
                import soundfile as sf
                sf.write(output_path, audio.reshape(-1).detach().cpu().numpy(), 24000)
            else:
